...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    console.error('[platform-events] fetch error', error);
    return NextResponse.json({ error: 'fetch_failed' }, { status: 500 });
  }
  return NextResponse.json({ events: (data ?? []) as PlatformEvent[] });
}
...
export async function POST(request: NextRequest) {
  ...
  const insertRow = { ...payload, created_by: auth.ctx.operatorId };
  ...
  if (error && isMissingRedemptionCodeColumn(error)) {
    const { redemption_code: _omit, ...withoutCode } = insertRow;
  ...
  if (error) {
    ...
    console.error('[platform-events] create error', error);
    return NextResponse.json({ error: 'save_failed' }, { status: 500 });
  }
  return NextResponse.json({ event: data as PlatformEvent }, { status: 201 });
}
